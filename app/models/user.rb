...
class User < CouchRestRails::Document
  ...
  def add_mobile_login_event imei, mobile_number
    self.login_history << MobileLoginEvent.new(:imei => imei, :mobile_number => mobile_number)
  end

  def add_web_login_event ip_address
    self.login_history << WebLoginEvent.new(:ip => ip_address)
  end
